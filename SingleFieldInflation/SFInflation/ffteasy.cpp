...
void fftc1(double f_arg[], int N_arg, int skip, int forward)
{
  ...
  struct complex *c = (struct complex *)f_arg; /* Treat f as an array of N complex numbers */
  ...
  for(dim=ndims-1;dim>=0;dim--) /* Loop over dimensions */
  {
    ...
    for(i=0;i<totalsize;i+=planesize) /* Take big steps to begin loops of transforms */
      for(j=0;j<skip;j++) /* Skip sets the number of transforms in between big steps as well as the skip between elements */
        fftc1(f_arg+2*(i+j),size[dim],skip,forward); /* 1-D Fourier transform. (Factor of two converts complex index to double index.) */
...
void fftr1(double f_arg[], int N_arg, int forward)
{
  ...
  struct complex temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complex *c = (struct complex *)f_arg; /* Treat f as an array of N/2 complex numbers */

  if(forward==1)
    fftc1(f_arg,N_arg/2,1,1); /* Do a transform of f as if it were N/2 complex points */
...
void fftrn(double f_arg[], double fnyquist[], int ndims, int size[], int forward)
{
  ...
  struct complex wb; /* wb = W^b = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
  struct complex temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complex *c = (struct complex *)f_arg, *cnyquist = (struct complex *)fnyquist; /* Treat f and fnyquist as arrays of complex numbers */
  ...
  {
    fftcn(f_arg,ndims,size,1); /* Do a transform of f as if it were N/2 complex points */
