...
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
double gradient_energy(int fld, int kloop_arg, FSUB_ARG)
{
  int i=0,j=0,k=0;
  double gradient=0.;
    ...
    #pragma omp parallel for reduction(-:gradient) num_threads(num_thread) private(j,k)
      for(i=1;i<N-1;i++)
        for(j=1;j<N-1;j++)
          for(k=1;k<N-1;k++)
          {
            gradient -= FIELD(fld)*lapl(fld,i,j,k,kloop_arg,FSUB);
          }
    #pragma omp parallel for reduction(-:gradient) num_threads(num_thread) private(j)
      for(i=0;i<N;i++)
        for(j=0;j<N;j++)
        {
          gradient -= f[fld][i][j][0]*laplb(fld,i,j,0,kloop_arg,FSUB) + f[fld][i][j][N-1]*laplb(fld,i,j,N-1,kloop_arg,FSUB); // z=0,N-1
          if(j==0 || j==N-1) continue;
          gradient -= f[fld][i][0][j]*laplb(fld,i,0,j,kloop_arg,FSUB) + f[fld][i][N-1][j]*laplb(fld,i,N-1,j,kloop_arg,FSUB); // y=0,N-1
          if(i==0 || i==N-1) continue;
          gradient -= f[fld][0][i][j]*laplb(fld,0,i,j,kloop_arg,FSUB) + f[fld][N-1][i][j]*laplb(fld,N-1,i,j,kloop_arg,FSUB); // x=0,N-1
        }
      break;
    case 2:
    case 4:
    #pragma omp parallel for reduction(-:gradient) num_threads(num_thread) private(j,k)
      for(i=1;i<N-1;i++)
        for(j=1;j<N-1;j++)
          for(k=1;k<N-1;k++)
          {
            gradient -= FIELDSUB1(fld)*lapl(fld,i,j,k,kloop_arg,FSUB);
          }
    #pragma omp parallel for reduction(-:gradient) num_threads(num_thread) private(j)
      for(i=0;i<N;i++)
        for(j=0;j<N;j++)
        {
          gradient -= fsub1[fld][i][j][0]*laplb(fld,i,j,0,kloop_arg,FSUB) + fsub1[fld][i][j][N-1]*laplb(fld,i,j,N-1,kloop_arg,FSUB); // z=0,N-1
          if(j==0 || j==N-1) continue;
          gradient -= fsub1[fld][i][0][j]*laplb(fld,i,0,j,kloop_arg,FSUB) + fsub1[fld][i][N-1][j]*laplb(fld,i,N-1,j,kloop_arg,FSUB); // y=0,N-1
          if(i==0 || i==N-1) continue;
          gradient -= fsub1[fld][0][i][j]*laplb(fld,0,i,j,kloop_arg,FSUB) + fsub1[fld][N-1][i][j]*laplb(fld,N-1,i,j,kloop_arg,FSUB); // x=0,N-1
        }
      break;
    case 3:
    #pragma omp parallel for reduction(-:gradient) num_threads(num_thread) private(j,k)
      for(i=1;i<N-1;i++)
        for(j=1;j<N-1;j++)
          for(k=1;k<N-1;k++)
          {
            gradient -= FIELDSUB2(fld)*lapl(fld,i,j,k,kloop_arg,FSUB);
          }
    #pragma omp parallel for reduction(-:gradient) num_threads(num_thread) private(j)    
      for(i=0;i<N;i++)
        for(j=0;j<N;j++)
        {
          gradient -= fsub2[fld][i][j][0]*laplb(fld,i,j,0,kloop_arg,FSUB) + fsub2[fld][i][j][N-1]*laplb(fld,i,j,N-1,kloop_arg,FSUB); // z=0,N-1
          if(j==0 || j==N-1) continue;
          gradient -= fsub2[fld][i][0][j]*laplb(fld,i,0,j,kloop_arg,FSUB) + fsub2[fld][i][N-1][j]*laplb(fld,i,N-1,j,kloop_arg,FSUB); // y=0,N-1
          if(i==0 || i==N-1) continue;
          gradient -= fsub2[fld][0][i][j]*laplb(fld,0,i,j,kloop_arg,FSUB) + fsub2[fld][N-1][i][j]*laplb(fld,N-1,i,j,kloop_arg,FSUB); // x=0,N-1
        }
  }
...
double stress_3pressure(int kloop_arg, SUB_ARG)
{
  ...
  switch(kloop_arg)
  {
    ...
    #pragma omp parallel for reduction(-:rho_3p) num_threads(num_thread) private(j,k)
      ...
      rho_3p += 2.*gradient_energy(0,kloop_arg,FSUB)/pow(a,2.*rescale_s+2.);
    ...
    #pragma omp parallel for reduction(-:rho_3p) num_threads(num_thread) private(j,k)
      ...
      rho_3p += 2.*gradient_energy(0,kloop_arg,FSUB)/pow(asub1,2.*rescale_s+2.);
      rho_3p += 4.*potential_energy(kloop_arg,FSUB);
      break;
    case 3:
    #pragma omp parallel for reduction(-:rho_3p) num_threads(num_thread) private(j,k)
      ...
      rho_3p += 2.*gradient_energy(0,kloop_arg,FSUB)/pow(asub2,2.*rescale_s+2.);
      rho_3p += 4.*potential_energy(kloop_arg,FSUB);
      break;
  }
