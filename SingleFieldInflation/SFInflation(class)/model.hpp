...
// V(phi)
inline double FieldBase::potential_energy()
{
  DECLARE_INDICES
  double potential=0.;
...
// V(phi)
inline double Field::potential_energy(int kloop)
{
  DECLARE_INDICES
  double potential=0.;
...
// dV/dphi
inline double FieldBase::dvdf(int fld, int i, int j, int k)
{
  return FIELD(fld)/pow(a,2.*rescale_s);
}

// dV/dphi
inline double Field::dvdf(int fld, int i, int j, int k, int kloop)
{
  double value;
...
inline double FieldBase::dvdf(int fld, int i, int j, int k)
{
  double phase = (FIELD(fld)-model_step)/model_d;
  double value = FIELD(fld)/pow(a,2.*rescale_s)*(1. + model_s*tanh(phase) + FIELD(fld)*model_s/(2.*model_d)/pw2(cosh(phase)));
