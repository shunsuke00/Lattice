...
*/
void fftc1ini(double f_arg[], int N_arg, int skip, int forward)
{
  int b,index1,index2,trans_size,trans;
  double pi2 = 4.*asin(1.);
  double pi2n,cospi2n,sinpi2n; /* Used in recursive formulas for Re(W^b) and Im(W^b) */
  struct complexini wb; /* wk = W^k = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
  struct complexini temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complexini *c = (struct complexini *)f_arg; /* Treat f as an array of N complex numbers */
...
Forward determines whether to do a forward transform (1) or an inverse one(-1)
*/
void fftcnini(double f_arg[], int ndims, int size[], int forward)
{
  int i,j,dim;
  ...
  for(dim=ndims-1;dim>=0;dim--) /* Loop over dimensions */
  {
    ...
    for(i=0;i<totalsize;i+=planesize) /* Take big steps to begin loops of transforms */
      for(j=0;j<skip;j++) /* Skip sets the number of transforms in between big steps as well as the skip between elements */
        fftc1ini(f_arg+2*(i+j),size[dim],skip,forward); /* 1-D Fourier transform. (Factor of two converts complex index to double index.) */
...
*/
void fftr1ini(double f_arg[], int N_arg, int forward)
{
  int b;
  double pi2n = 4.*asin(1.)/N_arg,cospi2n=cos(pi2n),sinpi2n=sin(pi2n); /* pi2n = 2 Pi/N */
  struct complexini wb; /* wb = W^b = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
  struct complexini temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complexini *c = (struct complexini *)f_arg; /* Treat f as an array of N/2 complex numbers */

  if(forward==1)
    fftc1ini(f_arg,N_arg/2,1,1); /* Do a transform of f as if it were N/2 complex points */
...
Forward determines whether to do a forward transform (1) or an inverse one(-1)
*/
void fftrnini(double f_arg[], double fnyquist[], int ndims, int size[], int forward)
{
  int i,j,b;
  ...
  struct complexini wb; /* wb = W^b = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
  struct complexini temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complexini *c = (struct complexini *)f_arg, *cnyquist = (struct complexini *)fnyquist; /* Treat f and fnyquist as arrays of complex numbers */
  ...
  {
    fftcnini(f_arg,ndims,size,1); /* Do a transform of f as if it were N/2 complex points */
