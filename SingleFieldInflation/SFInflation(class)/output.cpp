...
void FieldBase::meansvars(int flush)
{
...
void FieldBase::scale(int flush)
{
  static FILE *sf_;
...
void FieldBase::energy()
{
  ...
  for(fld=0;fld<nflds;fld++)
  {
    grad_energy = 0.5*field_lapl(fld)/pow(a,2.*rescale_s+2.);
    total += grad_energy;
...
void FieldBase::spectra()
{
  ...
  for(fld=0;fld<nflds;fld++)
  {
    ...
    // FFT
    fftrnout((double *)f[fld],(double *)fnyquist,NDIMS,arraysize,1);
