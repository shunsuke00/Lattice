...
double amplitude_gfposi(double px, double py, double pz, int i, int j, int k)
{
  ...
  if(px==0. && py==0.)
  {
    amp = (pw2(f[2][i][j][2*k]+f[3][i][j][2*k+1]) + pw2(f[2][i][j][2*k+1]-f[3][i][j][2*k]))/2.;
  }
  else
  {
    real = 0.;
    ...
    // x-component
    real  = pola_x_re(px,py,pz)*f[2][i][j][2*k]   + pola_x_im(px,py,pz)*f[2][i][j][2*k+1];
    imag  = pola_x_re(px,py,pz)*f[2][i][j][2*k+1] - pola_x_im(px,py,pz)*f[2][i][j][2*k];

    // y-component
    real  += pola_y_re(px,py,pz)*f[3][i][j][2*k]   + pola_y_im(px,py,pz)*f[3][i][j][2*k+1];
    imag  += pola_y_re(px,py,pz)*f[3][i][j][2*k+1] - pola_y_im(px,py,pz)*f[3][i][j][2*k];

    // z-component
    real  += pola_z_re(px,py,pz)*f[4][i][j][2*k]   + pola_z_im(px,py,pz)*f[4][i][j][2*k+1];
    imag  += pola_z_re(px,py,pz)*f[4][i][j][2*k+1] - pola_z_im(px,py,pz)*f[4][i][j][2*k];
